...
namespace libmodule {
	...
	public:
		struct ModuleSections_t {
			...
			ModuleSections_t(ModuleSections_t&& other) noexcept
				: m_svSectionName(std::move(other.m_svSectionName)), m_pSectionBase(std::move(other.m_pSectionBase)),
				  m_nSectionSize(std::exchange(other.m_nSectionSize, 0)) {}
		...
		[[nodiscard]] CMemory FindPattern(const CMemory pPattern, const std::string_view szMask, const CMemory pStartAddress = nullptr,
										  const ModuleSections_t* pModuleSection = nullptr) const;
		[[nodiscard]] CMemory FindPattern(const std::string_view svPattern, const CMemory pStartAddress = nullptr,
										  const ModuleSections_t* pModuleSection = nullptr) const;
